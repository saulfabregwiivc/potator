...
void Int6502(M6502 *R, byte Type)
{
    ...
    if ((Type == INT_NMI) || ((Type == INT_IRQ) && !(R->P&I_FLAG)))
    {
        ...
        M_PUSH(R->P&~B_FLAG);
        R->P &= ~D_FLAG;
        //if (Type == R->IRequest) R->IRequest = INT_NONE;
...
word Run6502(M6502 *R)
{
    ...
    for (;;)
    {
        ...
        case 0x14: MM_Zp(M_TRB);        break; //MR_Zp(I); M_TRB(I); break; /* uso, FixByME */
        ...
        case 0x1C: MM_Ab(M_TRB);        break; //MR_Ab(I); M_TRB(I); break; /* uso, FixByME */
